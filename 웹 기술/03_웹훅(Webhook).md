...
> 콜백(Callback)과 웹훅(WebHook)

## 1. 콜백(Callback)

> 콜백은 함수나 URL을 미리 등록해뒀다가, 특정 시점/이벤트에 미리 등록한 것을 호출하는 범용적인 프로그래밍 개념을 말한다.

"이 일이 끝나면 다시 원래의 함수(코드, 루틴 등)을 불러줘"라는 식의 범용 프로그래밍 개념을 의미한다.

- 함수는 함수의 파라미터로 넣어두는 함수(콜백 함수), 이벤트 리스너, 비동기 처리 로직 등을 모두 포함한다.

콜백("다시 호출함")의 대상이 코드, 함수, HTTP URL, 이벤트 등 범위가 넓다.

### 1) 예시

#### (1) 콜백 함수 전달

asyncFunction을 호출할 때, 이 함수의 작업을 수행하고 나면 호출할 콜백 함수를 함수의 파라미터로 넘기는 예시

asyncFunction을 호출하고 나면 내부 로직을 수행하다가 끝나면 수행할 콜백 함수를 전달하는 것이다.

```java
void asyncFunction(Runnable callback) {
    // ... 어떤 작업 수행 ...
    callback.run(); // 끝나면 콜백 호출
}
```

#### (2) 콜백 url 등록 / 호출 (Callback URL)

웹 시스템 상에서도 어떤 이벤트 혹은 함수에 대해서 콜백 URL(Callback URL)이라는 것을 등록해두고, 해당 이벤트/함수가 완료되면 등록한 콜백 URL로 POST 요청을 보내는 패턴도 콜백 방식의 예시 중 하나이다.

즉, 콜백 URL은 호출한 이벤트/함수가 종료되고 나면 결과값 혹은 이벤트를 POST 방식으로 전달하기 위한 URL이다.

콜백 url 방식의 경우 웹훅 방식과 동일하다고 볼 수 있다. `웹훅이 특수한 형태의 HTTP 콜백`이라고 이해하면 된다.

### 2) 콜백과 웹훅의 차이

둘 다 함수/이벤트에 대한 트리거(호출)이라는 공통 분모가 있지만, 콜백이 웹훅보다 더 큰 범용적인 상위 프로그래밍 개념이며 웹훅은 반드시 HTTP 요청이어야 한다는 차이점이 존재한다.
...
> API는 클라이언트가 서버로 폴링(Polling) 방식으로 요청을 보내고, 서버는 요청에 응답하는 방식이다.
...
> 웹훅은 서버에서 특정 이벤트가 발생했을 때 서버가 미리 정의된 외부 URL로 HTTP 방식으로 요청을 보내는 역방향 API 방식을 말한다.

webhook은 api에 비해 실시간 통신이 필요하거나 이벤트 기반의 작업이 필요할 때 더 적합한 방식이다.

다만 webhook은 비동기 api 처리 방식과는 거리가 있다.

- 일반적인 비동기 api 방식은 클라이언트가 요청을 서버로 보내고(클라이언트 → 서버), 서버는 해당 작업을 다 끝내면 클라이언트에게 응답을 반환하는 형태(이 때, 클라이언트는 응답이 오기까지 대기하지 않음)이지만,
- 반면에 webhook은 아예 시작과 요청 방향이 반대(서버에서 이벤트 발생 시 서버 → 서버(클라이언트))이다.
