...
> 교착 상태; 일련의 프로세스들이 서로가 가진 자원(리소스)을 기다리며 blocked된 상태
...
프로세스가 현재 보유한 자원을 놓지 않고(Hold), 다른 자원을 기다리는(Wait) 경우 (자원을 가진 프로세스가 다른 자원을 기다릴 때, 현재 보유한 자원을 놓지 않고 계속 가지고 있는 경우)
...
만약 리소스 요청을 허락해줬을 때 데드락이 발생할 가능성이 있으면, 리소스를 할당해도 안전할 때까지 계속 요청을 거절하는 알고리즘
